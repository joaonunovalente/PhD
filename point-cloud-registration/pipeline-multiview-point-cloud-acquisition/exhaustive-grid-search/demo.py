...
def register(config):
    ...
    # Extract the test folder name (e.g., "test1" or "test2") from PCI_PATH
    test_folder = os.path.basename(os.path.dirname(PCI_PATH))  # Extracts "test1" from "../data/test1/depth_1.ply"

    # Define the new results folder path inside "/results/test1/"
    results_folder_path = osp.join("results", test_folder)
    ...
    #### SAVE SOLUTION  #############################################################
    print(f'Saving results in results/{test_folder} ...')
    ...
    # Extract filenames from paths (without extensions)
    pci_name = os.path.splitext(os.path.basename(PCI_PATH))[0]  # e.g., "depth_1"
    pcj_name = os.path.splitext(os.path.basename(PCJ_PATH))[0]  # e.g., "depth_2"

    # Create results filename dynamically
    results_filename = f"{pcj_name}_{pci_name}.pickle"

    # Define full path
    save_results_pth = osp.join(results_folder_path, results_filename)
